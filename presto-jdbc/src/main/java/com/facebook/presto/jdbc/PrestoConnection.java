        ...
        implements Connection
{
    ...
    {
        return queryExecutor.getServerInfo(httpUri,user,password);
    }

    PrestoStatementClient startQuery(String sql)
    {
        String source = firstNonNull(clientInfo.get("ApplicationName"), "presto-jdbc");

        PrestoClientSession session = new PrestoClientSession(
                httpUri,
                user,
                this.password,
                source,
