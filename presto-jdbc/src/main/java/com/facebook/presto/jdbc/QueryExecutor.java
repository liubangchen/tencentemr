...
class QueryExecutor
    ...
    public PrestoStatementClient startQuery(PrestoClientSession session, String query) {
        return new PrestoStatementClient(httpClient, queryInfoCodec, session, query);
    }
    ...
    public ServerInfo getServerInfo(URI server, String user, String password) {
        URI uri = uriBuilderFrom(server).replacePath("/v1/info").build();
        Request.Builder builder = prepareGet();
        builder.setHeader("Authorization", AuthManager.distAuthInfo(user, password));
        builder.setUri(uri);
